...
| Phase 18 | 前置：仓库主人确认 Phase 17，记录 D31–D33（分支清理改按包含关系，删除远端分支被拒绝，HTTP 403；取消 PR 的定时复查）。可配置的审批策略（ADR-030，D32）：`configs/approval_policy.yaml` 的规则按顺序匹配（工具名与场景 glob、风险级别、参数的数值比较与枚举成员），决策为 auto_approve / require_human / deny；加载时 schema 校验，未知字段报错并给出 YAML 路径；没有命中时 write 与 destructive 一律交人工；destructive 在加载、匹配、网关签发三处都不会被自动批准；auto_approve 的调用由网关以 `policy:<规则编号>` 签发令牌（绑定 `preview_unavailable`、不预演），执行后照常测量改动；deny 带令牌也拒绝；智能体按判定分流；审计、审批请求与 UI 记录并显示规则编号；新增 `workbench gateway policy test`；README 截图重新生成 | `0666427`、`083eed5`、`2357110`、`1d9b6cb`、`4d00084`、`8bfd2e8`、`7632928` 与本行所在的文档提交 |
