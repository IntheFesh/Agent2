...
|---|---|---|---|---|
| U1 | 用 vLLM 服务 Arctic-AWM | `configs/serving/arctic-awm-4b.yaml`、`scripts/serve_vllm.sh`、`workbench serve vllm-cmd`、compose 中的 `vllm` 服务（`gpu` profile） | 无 GPU。模型卡已于 2026-09-24 读取（`docs/verification/2026-09-24-model-cards.md`），但卡片没有指定 vLLM 的 tool-call parser，因此 parser 与 chat template 仍默认不设置。按 vLLM v0.19.0 源码，智能体的 act 请求带原生 `tools`、未指定 `tool_choice` 时默认为 `"auto"`，服务端未开启 `--enable-auto-tool-choice` 与 `--tool-call-parser` 会直接拒绝（`serve/render/serving.py:197-215`，ADR-018）；所以按当前 profile，vLLM 后端的第一次 act 调用预计失败，需要在 GPU 上确认并决定是否启用 `hermes` parser | Linux x86_64 + CUDA GPU、HF 访问。执行 `scripts/serve_vllm.sh`，再用 `WORKBENCH_LLM__BACKEND=vllm` 运行 `workbench agent run` |
