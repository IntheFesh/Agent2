...
| 7 | 2026-09-24（Phase 13） | `workbench synth run --scenarios 1 --scenario-file … --execute`：手写场景 `local_it_service_desk`，从 `gen task` 开始（ADR-021），10 个任务，verifier 用 sql 模式；包含一次故意中断与续跑 | 待填 | 待填 | 待填 | 约 ¥2–5。依据：官方数据中同类产物的大小，可见输出约 13 万 token，另有思考 token；输入约 10–14 万 token；另外留出重试和中断时丢失的在途请求。AWM 的输出上限参数对 DeepSeek 不生效（探针 P4），单个请求最多可输出 64K token，因此设止损：运行期间按账本（上界单价）累计，达到 ¥6 立即终止并停下报告。在途请求尚未进账本，最坏再多约 ¥5（10 个 verifier 请求同时在途），累计仍低于 ¥15 | 待填 | 待填 | 待填 |
