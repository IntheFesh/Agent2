...
- **后续（Phase 12.5）**：仓库主人决定修复。网关现在以离线目录中路由的 HTTP 方法为风险下限（ADR-015），官方数据集上 POST/PUT/PATCH/DELETE 工具中被判为 `read` 的从 38 个降到 0 个，这些工具也就不会再被标成 `empty`（`docs/verification/logs/2026-09-24-phase12.5-risk-floor.log`）。
